...
class StripePayment extends BD_Controller {
	...
	/*
	public function add_payment_old_post() 
    {
        $amount =0;
        ...
        try {
				...
				if (empty($this->input->post('card_id'))) {
					...
					$get_txnid = $this->Auth_model->getCustomFields(TABLES::HOLD_PAYMENT_HISTORY,array('user_id'=>$userid,'status'=>'1',),'*');
				  $get_txn_id = $get_txnid->row();
				  
					if(!empty($get_txn_id)){
				  //echo $get_txn_id->amount; die;
						...
						//print_r($payment); die;
						$id = $this->Auth_model->save(TABLES::HOLD_PAYMENT_HISTORY,array('user_id'=>$userid,'amount'=>$amount,'txn_id'=>$payment->id,'date'=>date('Y-m-d H:i:s')));

						if ($id){
							$affected= $this->Auth_model->update(TABLES::USER,array('user_id'=>$this->user_data->id),array('cancellation_charge'=>0));
							//$affected= $this->Auth_model->update(TABLES::RIDE,array('ride_id'=>$ride_id),array('payment_status'=>'COMPLETED','payment_mode'=>'ONLINE'));
							// $user_query=$this->Auth_model->getCustomFields(TABLES::USER,array('user_id'=>$this->user_data->id),'gcm_token');
							//$user_query = $this->Auth_model->get_current_ride($userid);
							$user_query=$this->Auth_model->getCustomFields(TABLES::USER,array('user_id'=>$this->user_data->id),'gcm_token');
							$load = array();

							$load['title']  = SITE_TITLE;

							$load['msg']    = 'Pre-authorized amount  has been added Successfully';
