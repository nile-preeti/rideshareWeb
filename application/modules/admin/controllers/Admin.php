    ...
    */
	public function login()
	{
		...
		if ($this->form_validation->run()==false) {
			...
			$res =$this->Admin_model->getCustomFields(Tables::ADMIN,array('username'=>$this->input->post('username')),'*');
			$flag =1;
			if ($res->num_rows()>0) {
				$row = $res->row();
...
*/
    public function forgot_password()
    {
        ...
        $query = $this->Admin_model->getCustomFields(Tables::ADMIN,array('username'=>$this->input->post('email')),'username,id');
        if ($query->num_rows()>0) {
	 ...
	 public function map($par = NULL, $par2 = NULL) {
        ...
        if (!empty($session->id)){
            if (!empty($par)) {
                ...
                $qry = $this->db->query("select u.*,hph.amount as hold_amount,hph.rest_amount,r.txn_id, r.drop_lat,r.drop_long,r.pickup_lat,r.pickup_long,r.pickup_adress,r.pikup_location as pickup_location,vd.vehicle_no,vd.color,r.drop_locatoin as drop_location,r.drop_address,r.amount,d.name as driver_name,d.email as driver_email,r.distance,r.driver_id,r.user_id from rides r join users d on r.driver_id = d.user_id join users u on u.user_id=r.user_id join hold_payment_history hph on hph.txn_id=r.txn_id join vehicle_detail as vd on vd.user_id=r.driver_id where ride_id = $par");
   ...
   // echo $type;
    //die;
        $device_token='0'; 
        $device_id='0';
       $data=$this->Admin_model->update(Tables::USER,array('user_id'=>encrypt_decrypt('decrypt',$id)),array('device_type'=>$device_id,'device_token'=>$device_token));
        $this->Admin_model->update(Tables::USER,array('user_id'=>encrypt_decrypt('decrypt',$id)),array('is_logged_in'=>2));
...
*/
    public function update_user()
    {
        ...
        if($query->num_rows()){
            ...
            $num   = $this->Admin_model->getSingleRecord(Tables::USER, array('mobile' => $this->input->post('mobile')));
             ...
             if ($total_count>$flag){
                echo json_encode(array('status'=>true,'message'=>'This Mobile number has already been registered.'));
             }else{
    ...
    */
    public function delete_user()
    {
        //print_r($_POST);
        $user_id =$this->input->post('user_id');
        $ride_query = $this->Admin_model->getCustomFields(Tables::RIDE,array('user_id'=>$user_id),'ride_id');
...
*/
    ...
    */
    ...
    //     //$config["total_rows"] = $this->Admin_model->get_ride_list($filter_data,NULL,NULL)->num_rows();
    //     $config["total_rows"] = $this->Admin_model->get_audio_list($filter_data,NULL,NULL)->num_rows();
    //     $config["per_page"]             = (isset($filter_data['per_page']) && !empty($filter_data['per_page']) && is_numeric($filter_data['per_page']))?$filter_data['per_page']:PER_PAGE;
    //     $config['query_string_segment'] = 'page';
    //     $this->pagination->initialize($config);
    //     $page                   = (isset($_GET['page']) && intval($_GET['page'])) ? $_GET['page'] : 0;
    ...
    //     $page                   = ($page) ? (($page - 1) * $config["per_page"]) : 0;
    //     $data["links"] = $this->pagination->create_links();        
    //     $data["result"] = $this->Admin_model->get_audio_list( $filter_data,$config["per_page"],$page)->result();
...
*/
    ...
    */
    ...
    public function update_passpord() {       
        ...
        //unset($post_data['user_id']);
        //$affected_row = $this->Admin_model->updatepassword(Tables::USER,array('user_id'=>$user_id),$post_data);
    ...
    */
    public function vehicle_type_list(){
        ...
        $filter_data             = $this->input->get();
        $data['filter_data']     = $filter_data;
        $data['types']           = $this->Admin_model->getCustomFields(Tables::VEHICLETYPE,'1=1','id,title,status,created_date,short_description');
        $data['vhecile']           = $this->Admin_model->getCustomFields(Tables::VEHICLE_SUBCATEGORY_TYPE,'1=1','*');
    ...
    */
    public function add_vehicle_type(){   
        ...
        $post_data['updated_date'] =date('Y-m-d H:i:s');
        $post_data['status'] =1;
        $id = $this->Admin_model->save(Tables::VEHICLETYPE,$post_data);
        if ($id) {
	...
	public function update_vehicle_category(){
        //$post_data = $_FILES['car_pic']['name'];
        $post_data = $this->input->post();
        //print_r($post_data['id']);die;
...
*/
						...
						$url= $_SERVER['HTTP_REFERER'];
							...
							$payout_status = $this->Admin_model->save('payout_status',$data);
							 ...
							 if($affected_row){
								 ...
								 //$this->session->set_flashdata('message', '<p class="center">Article added successfully</p><p class="center">'.$payout_data['id'].'</p>');
									redirect('admin/payout/'.encrypt_decrypt('encrypt',$driver_id) );
								/* $data = $this->Admin_model->save('payout_status',$data);							
								if($data){
									$this->session->set_flashdata('success_msg','Driver payout created successfully.');
									redirect('admin/payout/'.encrypt_decrypt('encrypt',$driver_id) );
								} */
							 }else{
								$this->session->set_flashdata('error_msg','Stripe details are pending to submit.');
	...
	public function delete_stripe_account() {       
		...
		$this->load->library('common/stripe_lib');
		$all_accounts = $this->stripe_lib->deleteAccount('acct_1OhVZ7BRMk5I0EYY');
		echo "<pre>";
		print_r($all_accounts); die;
	}
