...
class Auth_model extends MY_Model{
    ...
    public function get_nearby_driver(){ 
        empty($_GET['limit']) ? $limit = 5000 : $limit = $_GET['limit'];
    	$this->db->select("u.user_id,u.name,u.email,u.latitude,u.longitude,(((acos(sin((" . $_GET['lat'] . "*pi()/180)) *
...

    {
        ...
        $this->db->select('sum(r.amount) as earning_amount,r.is_technical_issue');
    ...
    {
       $this->db->select('u.user_id,u.name_title,u.last_name,u.name,u.email,u.identification_document_id,id.document_name as identification_document_name,u.identification_issue_date,u.identification_expiry_date,u.verification_id,u.country_code,u.mobile,u.country,u.state,u.city,IFNULL(CONCAT("'.base_url('uploads/profile_image/'.$user_data->id.'/').'",u.avatar)," ") as profile_pic,IFNULL(CONCAT("'.base_url('uploads/verification_document/'.$user_data->id.'/').'",u.verification_id)," ") as verification_id,u.avatar as profile_image,IFNULL(ROUND(avg(f.rating),1),0) as total_rating, IFNULL(ROUND(avg(rf.rating),1),0) as rider_total_rating');
       $this->db->from(Tables::USER.' u'); 
    ...
    public function getvehicle_byCategory($catid){
        ...
        $this->db->join(' vehicle_subcategory_type vst','vt.id=vst.vehicle_type_category_id','left');
