...
use \Firebase\JWT\JWT;class Auth extends BD_Controller {
    ...
    public function register_post()
    {
       ...
       /* if($this->input->post('utype')==2 && $age<18){
        ...
        if ($this->form_validation->run()==false) {
            ...
            ///http://icanstudioz.com/taxiapp/user/register/format/json?country=India&city=Mathura&latitude=27.488692&mobile=9808518726&utype=1&email=muneshsingh12@gmail.com&password=abc@123&name=Munesh&state=Uttar Pradesh&gcm_token=dQMl-SPYZUo:APA91bH7CdFXeUt9iSPn8aStz4jhlF8FM4yM6pV-U2mYqc38jPSFvQleizg3g2gvL5fvxmDpQ_KXsPCj8VpzzXt0-7ZeGvfD2PykPeJr_Ew3CDU_vBgYTaaiLm4NVW3pvTrOoooe7LHj&longitude=77.6477062?fbffb=dbfbb
    ...
    function getAddressDetail1($lat='',$long='')
    {       
        //$lat=25.248275849703724 ;
        //$long=83.35504111300732;
        //$lat=33.7737735;
        //$long=-117.9279083;
        //$lat=41.88077884867035;
        //$long= -87.60524656326443;
        //AIzaSyCO1Fh9gjLTVh8iWc2yaVjv2kvGym31LVI";    
    ...
    public function get_vehicle_category_post()
    {
        ...
        foreach($res1 as $list){ 
            ...
            if(!empty($list->id)){              
                ...
                //$category_year = $this->Auth_model->category_year($list->id);
				
                //$res = $this->Auth_model->getCustomFields(Tables::VEHICLE_SUBCATEGORY_TYPE,array('status'=>1,"rate!="=> "0.00"),'id,title,rate,short_description,CONCAT("'.base_url('uploads/vehicle_type_image/').'",car_pic) as car_pic');
                $post_data = $this->input->post(); 
                //print_r($post_data);
                $flag=0;
                $surge_percentage = 0;
                if (isset($post_data['pickup_lat'])){  
                   ...
                   $api_data = $this->get_distance1($lat1,$long1,$lat2,$long2);
                    ...
                    if ($api_data['status']) {
                        ...
                        //$distance=str_replace(',','',$distanced);//comment for stastic distance(11-12-2023)END
                      //  $distance=$distanced;
                        ...
                        foreach ($res as $row_arr) {
							...
							$total_amount1=(string)preg_replace('/[^0-9\.]/ui','',$total_amount);
                            $row[] =array(
                                ...
                                'hold_amount'=>$row_arr->hold_amount,
        ...
        //$result_data['distance']  = $distance_arr;
        $this->response(array("status" => true, 'message'=>'',"data" =>  $vehcle_arr,'distance'=> str_replace(',','',$distance),'unit'=>'Miles','flag'=>$flag,'surge_percentage'=>$surge_percentage));
    }
    ...
    public function get_distance($address1='',$address2='')
    { 
        ...
        //echo $output['rows'][0]['elements'][0]['status'];
        //print_r($output);//die;        
        if ($output['rows'][0]['elements'][0]['status']!=(string)'Ok') {           
            $arr = array(
    ...
    // For getting driver distance
     function GetDrivingDistance_get($lat1, $long1,$destination='')
    { 
        // Create a new CURL instance<br>
        $ch = curl_init();
    ...
    // For getting distance via lat long
