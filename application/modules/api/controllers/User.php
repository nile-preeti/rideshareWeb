...
class User extends BD_Controller {
...

    {
        ...
        //'AAAAyNXjWVE:APA91bGDuZw6gWlSFSvQaoYP8Qv0iFF000MQYYw5l0av9pfKhNd2t0w5M946HGbat9xPaIyJVD2h6E3Sh_lO6HFOzrpmCg50IwjWZ9XJUZ3USbLQhPkFVofX85K9Xf_WPXAkXfgOUixn'      
    ...
    public function postRideToDriver_post(){
        ...
        if($user_detail_query->num_rows()>0){
            ...
            /* if(strtotime($user_detail->profile_upload_date)<strtotime(date('Y-m-d').' - 3 month')){
                $this->response(array("status" => false, 'message'=>'Your profile photo has been expired'));
            } */
			
				if(strtotime($user_detail->identification_expiry_date)<strtotime(date('Y-m-d'))){
                $this->response(array("status" => false, 'message'=>'Your identificaton document has been expired'));
				} 
        ...
        //echo $data[0]->cancellation_fee; die;
       ...
       // echo $this->db->last_query();die;
        ...
        /* if ($res_query->num_rows()==0) {
            foreach ($vehicle_arr->result() as $vehicle_row) { 
                $res_query = $this->Auth_model->getDriverFromUser($lat,$long,$distance,$vehicle_row->id);
                $vehicle_type_id=$vehicle_row->id;
                if ($res_query->num_rows()==0) {
                   continue;
                }else{
                    break;
        ...
        if ($res_query->num_rows()>0) {
				...
				//$rates = $this->db->get(Tables::VEHICLE_SUBCATEGORY_TYPE)->row();                
                //$date = date("Y/m/d");            
                //$daybydate = date('l', strtotime($date));
                //$dayname = strtolower($daybydate);
                //$rideCharges=$rates->$dayname;
            ...
            if ($ride_id) {
                ...
                //$api_driving_data= $authObj->GetDrivingDistance_get($res_query->row()->latitude,$res_query->row()->longitude,$res->pickup_adress);
    ...
    public function get_ride_status_post(){
        /*if ($this->user_data->utype==1) {
            ...
            $res = $this->Auth_model->getCustomFields(Tables::RIDE,array('ride_id'=>$this->input->post('ride_id'),'driver_id'=>$this->user_data->id),'*');
        ...
        if ($res->num_rows()>0) {
            ...
            //echo $this->db->last_query();
                ...
                $row->total_arrival_distance = bcdiv($APIDISTANCE,1,2);
                ...
                $row->total_distance = $api_data['distance'];
