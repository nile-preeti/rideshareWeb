...
class Driver extends BD_Controller {
    ...
    public function acceptRide_post(){		
        ...
        $waitingtime=$this->input->post('waitingtime');
    	$is_technical_issue = $this->input->post('is_technical_issue');
		$location_status = ($this->input->post('location_status')) ? ($this->input->post('location_status')) : "NO";
		$address_id=($this->input->post('stop_id')) ? ($this->input->post('stop_id')) :0;
        $qry = $this->Auth_model->get_current_ride($ride_id);
		/*#############################
            Getting free and paid waiting time from admin table  start  
        ###############################*/
		$waitTime = $this->Auth_model->getAdminData();				
		
		if($waitTime->num_rows()>0){
			$waitTimeAdmin = $waitTime->row();
		}
		$freeTimeInSecAtPickup =($waitTimeAdmin->free_waiting_time_pickup?$waitTimeAdmin->free_waiting_time_pickup*60:0);
        $freeTimeInSecAtStop   =($waitTimeAdmin->free_waiting_time_pickup?$waitTimeAdmin->free_waiting_time_stop*60:0);
        $freeTimeInSecAtDrop   =($waitTimeAdmin->free_waiting_time_pickup?$waitTimeAdmin->free_waiting_time_drop*60:0);

        $paidTimeInSecAtPickup =($waitTimeAdmin->paid_waiting_time_pickup?$waitTimeAdmin->paid_waiting_time_pickup*60:0);
        $paidTimeInSecAtStop   =($waitTimeAdmin->paid_waiting_time_stop?$waitTimeAdmin->paid_waiting_time_stop*60:0);
        $paidTimeInSecAtDrop   =($waitTimeAdmin->paid_waiting_time_drop?$waitTimeAdmin->paid_waiting_time_drop*60:0);
		
		/*#############################
            Getting free and paid waiting time from admin table  end  
        ###############################*/
        ...
        if ((string)$qry->row()->ride_status=='CANCELLED') {
          $this->response(array("status" => false, 'message'=>'Ride has been cancelled already'));
            ...
            if ((string)$status=='PENDING') {
                ...
                $load['msg']    = 'You have a new ride request.';
                $load['action'] = 'PENDING';
            ...
            }else if ((string)$location_status=="NO" && (string)$status=='ACCEPTED') {
               
			   if ($this->user_data->utype==2){
                    $user_detail['count_cancelled_ride']=0;
                }                				
				
				//echo $freeTimeInSec.$paidTimeInSec;
                $vehicle_detail = $this->Auth_model->getCustomFields(Tables::VEHICLE_DETAIL,array('status'=>1,'user_id'=>$this->user_data->id),'id');
				
                $this->Auth_model->update(Tables::RIDE,$condition,array('vehicle_detail_id'=>$vehicle_detail->row()->id,'free_waiting_time_pickup'=>$freeTimeInSecAtPickup,'free_waiting_time_stop'=>$freeTimeInSecAtStop,'free_waiting_time_drop'=>$freeTimeInSecAtDrop,'paid_waiting_time_pickup'=>
				$paidTimeInSecAtPickup,'paid_waiting_time_stop'=>$paidTimeInSecAtStop,'paid_waiting_time_drop'=>$paidTimeInSecAtDrop,));
            ...
            }else if((string)$location_status=='YES' && (string)$status=='ACCEPTED'){	
				$load = array();
                $load['title']  = SITE_TITLE;
                $load['msg']    = $res->driver_name. '(Driver) is at pick-up location and your waiting time has started.';
                $load['action'] = 'DRIVER_AT_PICKUP_LOCATION';
                $load['ride_id'] = $ride_id;
                $load['driver_id'] = $qry->row()->driver_id;
                $token = $res->user_fcm;
                $this->common->android_push($token, $load, FCM_KEY);				   
                $this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('status'=>$status,'on_location'=>$location_status,'start_waiting_time_onpickup'=>date("Y-m-d H:i:s")));
                //echo $this->db->last_query();

            }
			$qry = $this->Auth_model->get_current_ride($ride_id);
			$result = $qry->row();
			$toatalFreeWaitTime=$result->free_waiting_time_pickup+$result->free_waiting_time_drop+$result->free_waiting_time_stop;	
			$totalPaidWaitTime=$result->paid_waiting_time_pickup+$result->paid_waiting_time_stop+$result->paid_waiting_time_drop;
			$toatlwaitTime= $toatalFreeWaitTime+$totalPaidWaitTime;
			
            if((string)$status=='START_RIDE' && $location_status=='YES'){			   
				//print_r($result); die;				
				if((string)$waitingtime > $toatlwaitTime){
					
					$parMinuteCharge= $res->baserate/60;
					//$paiditmeInMinutes=$result->paid_waiting_time_pickup/60;
					$totalwaitingCharges= $parMinuteCharge*$totalPaidWaitTime;
					
					$this->load->library( 'common/stripe_lib');
					$cancelPriceCents = ($totalwaitingCharges*100 ); 
					$postdata=array(
						'txn_id'=>$res->txn_id,
						'amount'=>$cancelPriceCents,
					);
					...
					if($charge['status']=='succeeded'){
						/*   
						$this->payment_on_pikupwaitingtime_over();
						*/
						$id = $this->Auth_model->save(TABLES::PAYMENT_HISTORY,array('ride_id'=>$ride_id,'amount'=>$totalwaitingCharges,'txn_id'=>$res->txn_id,'date'=>date('Y-m-d H:i:s'),'cancellation_charge'=>'0'));
						if ($id){					
							$queryamt = $this->Auth_model->getCustomFields(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),'*');
							$rowamt = $queryamt->row();
									
							$rest_amt= $rowamt->amount-$totalwaitingCharges;
							$affected= $this->Auth_model->update(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),array('status'=>'0','charge_amount'=>$totalwaitingCharges,'rest_amount'=>$rest_amt));
							
							$user_query = $this->Auth_model->get_current_ride($ride_id);
							$load = array();
							$load['title']  = SITE_TITLE;
							$load['msg']    = 'Your waiting  charge $'.$totalwaitingCharges.' has been debited successfully';					
						...
						$this->Auth_model->update(TABLES::RIDE,array('ride_id'=>$ride_id),array('status'=>'COMPLETED','payment_status'=>'COMPLETED','payment_mode'=>'ONLINE','amount'=>$totalwaitingCharges));
					}
				}else{
					$load = array();
					$load['title']  = SITE_TITLE;
					...
					$token = $res->user_fcm;
					$this->common->android_push($token, $load, FCM_KEY);
					/*function  for save time and charges on ride start  */
					
					$returndata=0;
					$returndata = $this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('waiting_time_for_ride'=>$waitingtime,'end_waiting_time_onpickup'=>date('Y-m-d H:i:s')));			
					
					$totalWaitTime=0;
					 if($returndata){
						$qry = $this->Auth_model->get_current_ride($ride_id);
						$res1 = $qry->row();
						$freeWaitingTime=$res1->free_waiting_time_pickup;
						$totalWaitTime =$this->timeDiffrence($res1->swtop,$res1->ewtop);
						
						  $chrgesOnPickup= 0;
						  $chargePerminute = 0;
							
						  $chargepersec=0;
							if($totalWaitTime>=$freeWaitingTime){
								$paidwaittime = $totalWaitTime-$freeWaitingTime;
							}else{
								$paidwaittime	 =1;
							}
						  //echo "paid waittime:" .$paidwaittime;
						  if($paidwaittime >0){						
							$chargePerminute =$res->baserate / 60;	
							$chargepersec = $chargePerminute / 60;
							$chrgesOnPickup = number_format($chargepersec * $paidwaittime,2);
						  }
						  
						$pickupchrges =number_format($chrgesOnPickup,'2');
						$this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('total_waiting_time_onpickup'=>$paidwaittime,'waiting_charge_onpickup'=>$pickupchrges));					
					}	
					
					/*function  for save time and charges on ride end  */		
				}
			...
			if(!empty($address_id)){				
				...
				if((string)$status=='MID_STOP' && $location_status=='AT_STOP'){					
					$this->Auth_model->update(Tables::MIDSTOPS,array('id'=>$address_id),array('start_waiting_time_onstop'=>date("Y-m-d H:i:s")));
					$this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('status'=>$status,'on_location'=>$location_status));
					
					$load = array();
					$load['title']  = SITE_TITLE;
					$load['msg']    = $res->driver_name. '(Driver) is at stop location and your waiting time has started.';
					$load['action'] = 'DRIVER_AT_STOP';
					$load['ride_id'] = $ride_id;
					$load['driver_id'] = $qry->row()->driver_id;
					$token = $res->user_fcm;
					$this->common->android_push($token, $load, FCM_KEY);				   
				...
				if((string)$status=='MID_STOP' && $location_status=='LEAVE_STOP'){
					if((string)$waitingtime > $toatlwaitTime){
						...
						$stop_id=$this->Auth_model->update(Tables::MIDSTOPS,array('id'=>$address_id),array('end_waiting_time_onstop'=>date("Y-m-d H:i:s")));
						$totalWaitTimeStop=0;
					
						if($stop_id){							
							$stop_data=$this->db->get_where(Tables::MIDSTOPS, array('id =' =>$address_id))->row();
							$qryres = $this->Auth_model->get_current_ride($ride_id);
							$res1 = $qryres->row();
							$freeWaitingTime=$res1->free_waiting_time_pickup;
							$totalWaitTimeStop =$this->timeDiffrence($stop_data->start_waiting_time_onstop,$stop_data->end_waiting_time_onstop);
						}
						...
						$chargepersec=0;
						if($totalWaitTimeStop >=$freeWaitingTime){
							$paidwaittime = $totalWaitTimeStop-$freeWaitingTime;
						}else{
							$paidwaittime	 =0;
						}
						...
						if($paidwaittime >0){
							$chargePerminute =$res->baserate / 60;	
							$chargepersec = $chargePerminute / 60;
							$chrgesOnStop = number_format($chargepersec * $paidwaittime,2);
						}
						  
						$stopchrges =number_format($chrgesOnStop,'2');
						$this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('status'=>$status,'on_location'=>$location_status,'total_waiting_time_onstop'=>$paidwaittime,'waiting_charge_onstop'=>$stopchrges));

						$load = array();
						$load['title']  = SITE_TITLE;
						$load['msg']    = $res->driver_name. '(Driver) Ride started from stops !.';
						$load['action'] = 'DRIVER_AT_STOP';
						$load['ride_id'] = $ride_id;
						$load['driver_id'] = $qry->row()->driver_id;
						$token = $res->user_fcm;
            ...
            if((string)$status=='START_RIDE' && $location_status=='AT_DESTINATION'){ 
                ...
                $freeWaitTime = $this->Auth_model->getAdminData();			
        
                if($freeWaitTime->num_rows()>0){					
                    $freeTime = $freeWaitTime->row();
                }		

                $load['title']  = SITE_TITLE;

                $load['msg']    = 'You have reached at your destination and your waiting time has started.';

                $load['action'] = 'AT_DESTINATION';
                ...
                if((string)$location_status=='AT_DESTINATION'){				
                    $this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('on_location'=>$location_status,'status'=>$status,'start_waiting_time_ondrop'=>date("Y-m-d H:i:s")));
                }
			...
			if ((string)$status=='COMPLETED' || (string)$status=='CANCELLED') {
                    ...
                    $currentqry = $this->Auth_model->get_current_ride($ride_id);
                    $ridestatus = $currentqry->row();
					...
					if ((string)$status=='COMPLETED'){
						if(!empty($is_technical_issue)){
                            ...
                            if((string)$waitingtime > $toatlwaitTime){
								...
								$chargePerminute=$res1->baserate / 60;
								$chargepersec =$chargePerminute / 60;
								$payamount = number_format($chargepersec * $totalPaidWaitTime,2);
								$paycharges= $res1->amount+$payamount;
								
								$this->load->library( 'common/stripe_lib');
								$itemPriceCents = ( $paycharges*100 ); 
								$postdata=array(
								'txn_id'=>$res->txn_id,
								...
								$id = $this->Auth_model->save(TABLES::PAYMENT_HISTORY,array('ride_id'=>$ride_id,'amount'=>$paycharges,'txn_id'=>$res->txn_id,'date'=>date('Y-m-d H:i:s'),'cancellation_charge'=>0));
								  if($id){
									$affected= $this->Auth_model->update(TABLES::USER,array('user_id'=>$res->userid),array('cancellation_charge'=>0));

									$affected= $this->Auth_model->update(TABLES::RIDE,array('ride_id'=>$ride_id),array('waiting_time_for_ride'=>$waitingtime,'payment_status'=>'COMPLETED','payment_mode'=>'ONLINE','amount'=>$paycharges,'cancellation_charge'=>0));
									$queryamt = $this->Auth_model->getCustomFields(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),'*');
									$rowamt = $queryamt->row();
									$rest_amt= $rowamt->amount-$paycharges;
									$affected= $this->Auth_model->update(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),array('status'=>'0','charge_amount'=>$paycharges,'rest_amount'=>$rest_amt));								  
									$user_query = $this->Auth_model->get_current_ride($ride_id);
									$load = array();
							...
							$returndata = $this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('end_waiting_time_ondrop'=>date('Y-m-d H:i:s')));
						
							$qry1 = $this->Auth_model->get_current_ride($ride_id);
							$result1 = $qry->row();
                               
							if(!empty($returndata) && ($result1->on_location=='AT_DESTINATION')){
								
								$qry = $this->Auth_model->get_current_ride($ride_id);
								$res1 = $qry->row();								
								 
									$freeWaitingTime =$res1->swtod;
									$waitTimeFromPickup = ($res1->total_waiting_time_onpickup>0?$res1->total_waiting_time_onpickup:0);
									$totalWaitTimeDrop =$this->timeDiffrence($res1->swtod,$res1->ewtod);
									
									//echo "total waittime on COMPLETED RIDE:" .$totalWaitTimeDrop; 
									
									$chrgesOnDrop= 0;
									$dropchrges=0;
									
									if($totalWaitTimeDrop>=$freeWaitingTime){
										
										$sumOfPickDropTime=$waitTimeFromPickup+$totalWaitTimeDrop;
										//$totalFreeTime=$freeWaitingTime*2;
										$totalWaitingTime=$freeWaitingTime+$sumOfPickDropTime;
										if($sumOfPickDropTime<=$totalWaitingTime){
											$chargeDropwaittime = $totalWaitTimeDrop-$freeWaitingTime;
										}else{
											//$extratime = $totalWaitTimeDrop-$freeWaitingTime;
											$chargeDropwaittime = $res1->paid_waiting_time_drop-$waitTimeFromPickup-$freeWaitingTime;
										}
										
									}else{
										$chargeDropwaittime=0;
									}
								
								
								if($chargeDropwaittime > 0){
			
									$chargePerminute=$res->baserate / 60;
									$chargepersec =$chargePerminute / 60;
									$dropchrges = number_format($chargepersec * $chargeDropwaittime,2);
								}else{
									$dropchrges=0;
								}
								
								$this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('total_waiting_time_ondrop'=>$chargeDropwaittime,'waiting_charge_ondrop'=>$dropchrges));
							}
						}
						
						if(!empty($is_technical_issue)&&(string)$status =='COMPLETED'){
							$this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('is_technical_issue'=>$is_technical_issue,'payment_status'=>'PENDING','cancellation_charge'=>0,'is_technical_issue'=>'Simple','status'=>'CANCELLED','ride_complete_time'=>date('Y-m-d H:i:s')));
						...
						$pickupCharge= ($res->waiting_charge_onpickup)?(str_replace(',','',$res->waiting_charge_onpickup)):"0";
						$dropCharge= ($res->waiting_charge_ondrop)?(str_replace(',','',$res->waiting_charge_ondrop)):"0";
						$totalWaitCharge=$pickupCharge+$dropCharge;
						
						$tax_chargeOnwaiting =number_format($totalWaitCharge*($res->taxes/100),2);
						$totalwaitCharges= $totalWaitCharge+$tax_chargeOnwaiting;
						$totalAmt=$res->amount+$totalwaitCharges;
						
						
						$this->load->library( 'common/stripe_lib');
						$itemPriceCents = ( $totalAmt*100 ); 
						$postdata=array(
							'txn_id'=>$res->txn_id,
							'amount'=>$itemPriceCents,
						);
						...
						if(empty($is_technical_issue)){
							...
							if($charge['status']=='succeeded'){		
								...
								$id = $this->Auth_model->save(TABLES::PAYMENT_HISTORY,array('ride_id'=>$ride_id,'amount'=>$totalAmt,'txn_id'=>$res->txn_id,'date'=>date('Y-m-d H:i:s'),'cancellation_charge'=>0));
								  if ($id){
									$affected= $this->Auth_model->update(TABLES::USER,array('user_id'=>$res->userid),array('cancellation_charge'=>0));

									$affected= $this->Auth_model->update(TABLES::RIDE,array('ride_id'=>$ride_id),array('waiting_time_for_ride'=>$waitingtime,'payment_status'=>'COMPLETED','payment_mode'=>'ONLINE','amount'=>$totalAmt,'cancellation_charge'=>0,'ride_complete_time'=>date('Y-m-d H:i:s')));
									$queryamt = $this->Auth_model->getCustomFields(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),'*');
									$rowamt = $queryamt->row();
									$rest_amt= $rowamt->amount-$totalAmt;
									$affected= $this->Auth_model->update(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),array('status'=>'0','charge_amount'=>$totalAmt,'rest_amount'=>$rest_amt));								  
									...
									$load['msg']    = 'Your ride payment of $'.$totalAmt. ' has been debited successfully.' ;
									...
									$driverAmt=$totalAmt-($totalAmt*($user_query->row()->admin_fee/100));

									$load['title']  = SITE_TITLE;
									
									$load['msg']    = 'Your ride payment of $'.number_format($driverAmt,2). ' has been received. Thank you!';
						...
						if ($this->user_data->utype==2){  
							...
							if($res->ride_status=='ACCEPTED'){
								...
								$this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('cancelled_by'=>$this->user_data->id,'cancelled_count'=>$res->cancelled_count+1,'cancellation_charge'=>'0','status'=>$status));                          
								$load = array();
							...
							if($this->user_data->utype==1){ 
								$confirmtimestamp = strtotime($qry->row()->confirmation_time.' + 4 minute'); 
								$currenttimestamp= strtotime(date('Y-m-d H:i:s'));
								//echo $currenttimestamp.' cunfirm'.$confirmtimestamp;
								...
								$vehicle_subcategory_id=$this->Auth_model->getCustomFields(Tables::RIDE,array('ride_id'=>$ride_id),'vehicle_type_id');
								   ...
								   $this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('status'=>'CANCELLED','cancelled_by'=>$this->user_data->id,'cancellation_time'=>date('Y-m-d H:i:s'),'cancellation_charge'=>$cancellation_amount,'amount'=>'0'));
								   
								   
								$this->load->library( 'common/stripe_lib');
								$cancelPriceCents = ( $cancellation_amount*100 ); 
								$postdata=array(
									'txn_id'=>$res->txn_id,
									'amount'=>$cancelPriceCents,
								);
									...
									if($charge['status']=='succeeded'){
										...
										$id = $this->Auth_model->save(TABLES::PAYMENT_HISTORY,array('ride_id'=>$ride_id,'amount'=>'0','txn_id'=>$res->txn_id,'date'=>date('Y-m-d H:i:s'),'cancellation_charge'=>$cancellation_amount));
										  if ($id){
											$affected= $this->Auth_model->update(TABLES::USER,array('user_id'=>$res->userid),array('cancellation_charge'=>$cancellation_amount));

											//$affected= $this->Auth_model->update(TABLES::RIDE,array('ride_id'=>$ride_id),array('payment_status'=>'COMPLETED','payment_mode'=>'ONLINE','amount'=>$totalAmt));
											$queryamt = $this->Auth_model->getCustomFields(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),'*');
											$rowamt = $queryamt->row();
											$rest_amt= $rowamt->amount-$cancellation_amount;
											$affected= $this->Auth_model->update(TABLES::HOLD_PAYMENT_HISTORY,array('txn_id'=>$res->txn_id),array('status'=>'0','charge_amount'=>$cancellation_amount,'rest_amount'=>$rest_amt));
											...
											$load['title']  = SITE_TITLE;
											$load['msg']    = 'Your cancellation charge of $'.$cancellation_amount. 'has been debited successfully.';
									...
									$this->Auth_model->update(Tables::RIDE,array('ride_id'=>$ride_id),array('status'=>'CANCELLED','cancelled_by'=>$this->user_data->id,'cancellation_time'=>date('Y-m-d H:i:s'),'amount'=>'0','cancellation_charge'=>'0'));
								}
							}                      
    ...
    public function update_lat_long_post($value='')
    {

       $affected =$this->Auth_model->update(Tables::USER,array('user_id'=>$this->user_data->id),array('latitude'=>$this->input->post('lat'),'longitude'=>$this->input->post('long')));

        if ($affected) {
            $this->response(array("status" => true, 'message'=>'Successfully.'));
        }else{
            $this->response(array("status" => false, 'message'=>'Error occured, Please try after some time'));
        }
    }
        ...
        $amount = $row->amount;
            ...
            //print_r( $api_driving_data);die;
			 $vehicle_image_query = $this->Auth_model->get_current_active_vehicle_image($row->driver_id);
				...
				$total_amount=(string)str_replace(',','',$amount);
				//echo $total_amount; die;
				$row->mobile= $getmobile;	
			//(string)$tamount;
			
			$row->start_waiting_time_onpickup= ($row->start_waiting_time_onpickup) ? ($row->start_waiting_time_onpickup) : "";
			$row->total_waiting_time_ondrop = ($row->total_waiting_time_ondrop) ? ($row->total_waiting_time_ondrop) : "";
			$row->start_waiting_time_ondrop=($row->start_waiting_time_ondrop) ? ($row->start_waiting_time_ondrop) : "";
			$row->end_waiting_time_ondrop=($row->end_waiting_time_ondrop) ? ($row->end_waiting_time_ondrop) : "";			
			$row->waiting_charge_ondrop=($row->waiting_charge_ondrop) ? ($row->waiting_charge_ondrop) : "";
			$row->last_waiting_time=($row->waiting_time_for_ride)?($row->waiting_time_for_ride) : "0";
			
			$currentTime=date("Y-m-d H:i:s");
			$timeDiiff = $this->timeToSec($row->start_waiting_time_onpickup,$currentTime);
			$timeDiiffOnDrop = $this->timeToSec($row->start_waiting_time_ondrop,$currentTime);
			...
			$row->totaltimeDiffrence  = (string)($timeDiiff) ? ((string)$timeDiiff) : "0";  // time in seconds 
			$row->totaltimeDiffrenceOnDrop  = (string)($timeDiiffOnDrop) ? ((string)$timeDiiffOnDrop) : "0";  // time in seconds 
			
			$row->stops = $this->Auth_model->get_stops($row->ride_id);
           ...
           $this->set_response(['status'=>true,'user_status'=>$user_detail->row()->status,'license_expiry'=>$license_expiry,'insurance_expiry'=>$insurance_expiry,'car_expiry'=>$car_expiry,'driver_rest_time'=>$driver_rest_time,'change_vehicle'=>$change_vehicle,'identification_expiry'=>$identification_expiry,'inspection_expiry'=>$inspection_expiry,'message'=>'','data'=>$row,'document_approval'=>$vehicle_detail],REST_Controller::HTTP_OK);
            ...
            $this->set_response(['status'=>false,'license_expiry'=>$license_expiry,'insurance_expiry'=>$insurance_expiry,'car_expiry'=>$car_expiry,'driver_rest_time'=>$driver_rest_time,'change_vehicle'=>$change_vehicle,'identification_expiry'=>$identification_expiry,'inspection_expiry'=>$inspection_expiry,'message'=>'','data'=> $query->result(),'document_approval'=>$vehicle_detail],REST_Controller::HTTP_OK);
       }
    }
	...
	public function give_feedbackForRider_post()
    {
        ...
        if($this->input->post('rating')==5){
            ...
            $load = array();
            $load['title']  = SITE_TITLE;
            $load['msg']    = 'Your Driver Rates you: '.$feedbackmsg;
            $load['action'] = 'feedback';
    ...
    public function driver_location_post(){
		...
		if($this->user_data->utype =='2'){
			...
			$user_id    	= $this->user_data->id;//$this->input->post('user_id');
			 ...
			 $authObj = modules::load('api/auth');
			 $api_data = $authObj->get_distance1($pickup_lat,$pickup_long,$drop_lat,$drop_long);
			 
			/* ---Log Section start--*/
			$obj = modules::load('admin');
			$obj->custom_log('API',' driver_location, User Id: '.$this->user_data->id);
			#--log End --
			$this->set_response($api_data, REST_Controller::HTTP_OK);
		}else{
			$this->set_response(array('status'=>true,'message'=>'unauthorized login attempt, Please try after some time'), REST_Controller::HTTP_OK);
	...
	/*##########################
	  Function timeToSec is using for given 2 dates to convert into seconds and return the time diffrence in seconds only  from two date 
	  Developer: Ram 
	############################*/

    
	public function timeToSec($time1,$time2){
		...
		$daysInMinute = $days*24*60;
		$hourInMinute = $hours*60;
		$totalMinutes = $daysInMinute+$hourInMinute+$minutes;
		$minutesInSec= $totalMinutes* 60;
		$totalSeconds = $minutesInSec + $seconds;
	...
	/*##########################
	  Function timeDiffrence is using for given 2 dates to convert into seconds and return the time diffrence in minutes and seconds only  from two date 
	  Developer: Ram 
	############################*/
	
	 public function timeDiffrence($time1,$time2){		
		...
		$hours = $interval->format('%h');
		$minutes = $interval->format('%i');
		$totalSeconds = $interval->format('%s');
		
		$hourInMinute = $hours*60;
		$totalMinutes = $hourInMinute+$minutes;
        $totalMinutesInSec=$totalMinutes*60;
		return $totalMinutesInSec+$totalSeconds;
