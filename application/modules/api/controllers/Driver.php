...
class Driver extends BD_Controller {
    ...
    public function get_last_ride_get(){
        ...
        /* echo intdiv(strtotime(date('Y-m-d H:i:s'))-strtotime($login_query->row()->login_time),3600);
        echo ' Hours ';
        echo intdiv((strtotime(date('Y-m-d H:i:s'))-strtotime($login_query->row()->login_time))%3600,60);
        echo ' Minutes'; */
        ...
        //if(strtotime($login_query->row()->identification_expiry_date)<strtotime(date('Y-m-d')))
       ...
       if ($query->num_rows()>0) {
		...
		echo $this->user_data->utype;
		die; */
		$getmobile='0';		
        $amount = $row->amount;
        //echo $amount; die;
			//$amount =$row->amount-((int)$row->amount*($row->AdminRide_charges/100));
            if ($this->user_data->utype==2) {
            ...
            $authObj = modules::load('api/auth');
            //$api_data= $authObj->get_distance($row->pickup_adress,$row->drop_address);
            $api_data= $authObj->get_distance1($row->pickup_lat,$row->pickup_long,$row->drop_lat,$row->drop_long);
            //$api_driving_data= $authObj->GetDrivingDistance_get($row->latitude,$row->longitude,$row->pickup_adress);
