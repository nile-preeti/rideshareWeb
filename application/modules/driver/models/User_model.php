...
*/
	...
	public function payout_historybyweek($table,$business_id){
       $query = $this->db->query('select year(time) as year, week(time) as week, sum(amount) as total_paid_amount,sum(tip_amount) as total_tip_amount from '.$table.' WHERE driver_id = "'.$business_id.'" and status="COMPLETED" and is_technical_issue !="Yes" group by year(time), week(time) '); 
        return $query->result();
	}
